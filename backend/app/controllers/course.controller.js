...
exports.createProduct = async (req, res, next) => {
  try {
    ...
    next(new ApiError("Lỗi tạo sản phẩm", 500));
  }
};
...
exports.getAllCat = async (req, res, next) => {
  try {
    const result = await productService.getCat();
    return res.send(result)
  } catch (error) {
    next(new ApiError("Không tìm thấy loại", 500));
  }
};

exports.createCat = async (req, res, next) => {
  try {
    const result = await productService.createCat(req.body);
    return res.send(result)
  } catch (error) {
    next(new ApiError("Không tạo đc loại", 500));
  }
};
