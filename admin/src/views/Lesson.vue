                    ...
                    <div class="col-12">
                        <div class="semibold pb-4">
                            ...
                            <div class="row align-items-center justify-content-between px-3 pt-2">
                                ...
                                <div class="px-4 ">
                                    <button class="btn btn-sm btn-secondary mx-4" @click="goToAddLesson">
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="col-2">Tên bài học</th>
                                    <th scope="col" class="col-2">Lớp</th>
                                    <th scope="col" class="col-2"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <LessonList v-if="filteredLessonsCount > 0" :lessons="paginatedLessons"
                                    v-model:activeIndex="activeIndex" :start-index="startIndex" />
                                <p v-else>Không có Bài học.</p>
                            </tbody>
                            <tfoot>
                                <tr>
...
export default {
    ...
    computed: {
        ...
        filteredLessons() {
            ...
            if (!lowercaseSearchText) return this.lessons;
            return this.lessons.filter((_lesson, index) =>
                this.lessonStrings[index].includes(lowercaseSearchText)
            );
        },
        activeLesson() {
            if (this.activeIndex < 0) return null;
            return this.filteredLessons[this.activeIndex];
        },
    ...
    methods: {
        async retrieveLessons() {
            try {
                this.lessons = await LessonService.getAllLesson()
            } catch (error) {
                console.log(error);
            }
        },
        ...
        sortLessons() {
            ...
            this.filteredLessons.sort(compareFn);
        },
        changeSort(field) {
